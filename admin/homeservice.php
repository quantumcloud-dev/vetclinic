      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box-header bg-gray">
            <i class="text-center" style="color:blue"><h4> Pending -Home Service- Requests</h4></i></div>
            <div class="box-body">
              <table id="pending" class="table table-striped table-bordered table-hover bg-gray" style="height:100%">
                ...
                <tbody>
                  <?php
                    $conn = $pdo->open();
                    try{
                      $stmt = $conn->prepare("SELECT * FROM requests where request_status='Pending' and appointment_type='Home Service' order by request_id DESC");
                      $stmt->execute();
                      foreach($stmt as $row){
                        echo "<tr>
